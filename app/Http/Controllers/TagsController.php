...
class TagsController extends UserController
{
    ...
    public function index()
    {
        ...
        $tags_db = Tags::with([
            ...
            ->orderBy('slug', 'asc') // Cannot orderBy 'tag' anymore directly
            ->get();
        $totalTags = count($tags_db);

        $tags = [];
        foreach ($tags_db as $tag) {
            $tagName = $tag->details->first()?->name ?? $tag->slug;
            $firstLetter = strtoupper(mb_substr($tagName, 0, 1));
    ...
    public function show($tag, $category_get = 'couplets')
    {
        ...
        // count poetry
        $categoriesWithCounts = Categories::withCount([
            'poetry' => function ($query) use ($tag) {
                $query->whereJsonContains('poetry_tags', $tag);
            }
        ])
            ->whereHas('poetry', function ($query) use ($tag) {
                $query->whereJsonContains('poetry_tags', $tag);
            })
            ->with([
                'detail' => function ($q) {
        ...
        $total_couplets = Couplets::whereJsonContains('couplet_tags', $tag)
            ->where(['poet_id' => $poetId, 'poetry_id' => 0])->count();
        ...
        if ($category !== null) {
            ...
            $tagName = $profile->details->first()?->name ?? $profile->slug;
            $title = trans_choice('labels.best_poetry_on_title_category', 1, ['title' => $tagName, 'category' => trans('labels.poetry')]);
        }
    ...
    public function load_more_poetry(Request $request)
    {
        ...
        $poetryQuery = ($request->category === 'couplets')
            ? Couplets::where(['poetry_id' => 0])->whereJsonContains('couplet_tags', $request->tag)
            ...
            if ($request->category === 'couplets') {
                ...
                foreach ($poetry as $index => $item) {
                    ...
                    if ($item->couplet_tags != NULL) {
                        ...
                        $usedTags = Tags::with([
                            'details' => function ($q) use ($item) {
                                $q->where('lang', $item->lang);
                            }
                        ])
                            ->whereIn('slug', $decodeTags)
                            ->get()
                            ->mapWithKeys(function ($tag) {
                                return [$tag->slug => $tag->details->first()?->name ?? $tag->slug];
                            })
                            ->toArray();
