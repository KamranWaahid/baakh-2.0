...
class PoetsController extends UserController
{
    ...
    public function index(Request $request)
    {
        ...
        $poet_tags = Tags::where(['type' => 'poets'])
            ->with([
    ...
    public function with_tags($tag)
    {
        ...
        $active_tag = Tags::where('slug', $tag)
            ->with([
                'details' => function ($q) use ($locale) {
                    $q->where('lang', $locale);
                }
        ...
        $this->SEO_General($active_tag->details->first()?->name ?? $active_tag->slug, 'This is description');
    ...
    public function with_slug($name, $category_get = 'all')
    {
        ...
        // count poetry
        $categoriesWithCounts = Categories::select(['id', 'slug'])->withCount([
            'poetry' => function ($query) use ($poetId, $locale) {
    ...
    public function load_more_poetry(Request $request)
    {
        ...
        $poetryQuery = ($request->category === 'couplets')
            ...
            : Poetry::where('category_id', $request->category_id)->with([
