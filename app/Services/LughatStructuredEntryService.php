...
class LughatStructuredEntryService
{
    public function build(LughatLemma $lemma): array
    {
        ...
        return [
            ...
            'sindhi_entry' => [
                ...
                'part_of_speech' => $lemma->pos ?: $senses->first(fn (LughatSense $sense) => filled($sense->part_of_speech))?->part_of_speech,
                'gender' => $lemma->morphology?->gender,
                'number' => $lemma->morphology?->number,
                'root' => $lemma->morphology?->root,
                'etymology' => $lemma->etymology,
                'meanings' => $senses
                    ->map(fn (LughatSense $sense, int $index) => $this->meaning($sense, $index))
                    ->values()
                    ->all(),
    ...
    private function meaning(LughatSense $sense, int $index): array
    {
        return [
    ...
    {
        return $relations
            ->filter(fn (LughatRelation $relation) => in_array($relation->relation_type, ['related', 'hypernym'], true))
            ->map(fn (LughatRelation $relation) => [
                'word' => $this->relationWord($relation),
                'romanization' => $relation->romanization ?? $relation->relatedLemma?->transliteration,
    ...
    private function relationWord(LughatRelation $relation): string
    {
        return $relation->relatedLemma?->lemma ?? $relation->related_word;
    }

    private function englishIndex(LughatLemma $lemma, Collection $senses): array
    {
        return $senses
            ->flatMap(function (LughatSense $sense, int $index) use ($lemma) {
                ...
                return collect($equivalents)
                    ...
                    ->map(fn (string $englishWord) => [
                        ...
                        'sense_note' => $sense->usage_label ?? $sense->short_gloss ?? ('LughatSense ' . (($sense->sense_order ?: $index + 1))),
    ...
    private function searchKeywords(LughatLemma $lemma, Collection $senses): array
    {
        ...
        return [
            ...
            'english' => $this->unique([
                ...$senses->flatMap(fn (LughatSense $sense) => $this->englishEquivalents($sense))->all(),
                ...($stored['english'] ?? []),
    ...
    private function metadata(LughatLemma $lemma): array
    {
    ...
    private function englishEquivalents(LughatSense $sense): array
    {
