...
const LughatQA = () => {
    const { data: report, isLoading } = useQuery({
        queryKey: ['lughat-qa-report'],
        queryFn: async () => {
            const res = await api.get('/api/admin/lughat/qa');
            return res.data;
        }
    });
            ...
            {items.length > 0 ? items.map((lemma) => (
                <div key={lemma.id} className="flex items-center justify-between rounded-lg border p-3">
                    ...
                    <Button size="sm" variant="outline" asChild>
                        <Link to={`/admin/baakh-lughat/lemmas/${lemma.id}`}>Review</Link>
                    </Button>
                </div>
            )) : (
        ...
        <CardContent className="space-y-3">
            {items.length > 0 ? items.map((sense) => (
                <div key={`${title}-${sense.id}`} className="flex items-center justify-between rounded-lg border p-3">
                    ...
                    {sense.lemma_id && (
                        <Button size="sm" variant="outline" asChild>
                            <Link to={`/admin/baakh-lughat/lemmas/${sense.lemma_id}`}>Review</Link>
                        </Button>
                    )}
                </div>
            )) : (
