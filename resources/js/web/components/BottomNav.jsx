...
const BottomNav = ({ lang }) => {
    ...
    return (
        <nav className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100/50 flex lg:hidden items-center justify-around px-4 z-50 shadow-[0_-8px_30_rgb(0,0,0,0.04)] active:shadow-none transition-[transform,opacity] duration-300 will-change-transform ${isHidden ? 'translate-y-[100%] opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`} style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 8px) + 8px)', paddingTop: '8px' }}>
            {navItems.map((item) => (
                <NavLink
                    key={item.path}
                    to={item.path}
