...
const PoemActionBar = ({ poem, lang, className, leftContent }) => {
    ...
    const [clapCount, setClapCount] = useState(poem.likes || 0);
    const [isLiked, setIsLiked] = useState(poem.is_liked || false);
    const [isSaved, setIsSaved] = useState(poem.is_bookmarked || false);
    ...
    return (
        <div className={`flex items-center text-gray-500 w-full ${className && className.includes('justify-center') ? 'justify-center' : 'justify-between'} ${className || ''}`} onClick={(e) => e.preventDefault()}>
            <div className={`flex items-center ${className && className.includes('justify-center') ? 'gap-3' : 'gap-4'}`}>
                {leftContent && (
                ...
                <button
                    onClick={handleClap}
                    className="flex items-center gap-2 hover:text-black active:scale-95 transition-all z-10 relative min-h-[44px] min-w-[44px] justify-center rounded-lg"
                    aria-label={`Clap for this poem (${clapCount} claps)`}
            ...
            <div className={`flex items-center ${className && className.includes('justify-center') ? 'gap-6' : 'gap-4'}`}>
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button
                            className="hover:text-black outline-none z-10 relative min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg"
                ...
                >
                    <Bookmark
                        className={`w-6 h-6 transition-all duration-300 ${isSaved ? 'fill-current scale-110' : 'scale-100'}`}
                        strokeWidth={1.5}
                    />
                </button>
