...
@section('content')
    <div class="row">
        <div class="col-6 m-auto">
            <div class="card">
                <form action="{{ route('admin.users.update', $profile->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        {{-- Image Field --}}
                        <div class="row d-flex flex-column justify-content-center align-items-center align-content-center">
                            <div class="image">
                                <img src="{{ file_exists($profile->avatar) ? asset($profile->avatar) : asset('assets/img/placeholder290x293.jpg') }}" class="rounded-circle" id="displayImage" width="200px" alt=""> 
                            </div>
                        ...
                        <div class="col-6 m-auto" id="user_role">
                            ...
                            <select name="roles" class="form-control" id="roles">
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" data-role_name="{{ Str::lower($role->name) }}" {{ $profile->hasRole($role->name) ? 'selected' : '' }} >{{ $role->name }}</option>
                                @endforeach
                            </select>
                            <input type="hidden" name="roles_name" id="roles_name" value="{{ $profile->role }}">
                        </div>
                            ...
                            <div class="col-12" data-for="name">
                                <div class="input-group mb-3">
                                    <input type="text" name="name"  class="form-control @error('name') is-invalid @enderror"
                                        value="{{ old('name', $profile->name) }}" placeholder="{{ __('adminlte::adminlte.full_name') }}" autofocus>
                            ...
                            <div class="col-12" data-for="name_sd">
                                <div class="input-group mb-3">
                                    <input type="text" name="name_sd" class="form-control @error('name_sd') is-invalid @enderror"
                                        value="{{ old('name_sd', $profile->name_sd) }}" placeholder="Full Name in Sindhi">
                        ...
                        <div class="input-group mb-3">
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                                value="{{ old('email', $profile->email) }}" placeholder="{{ __('adminlte::adminlte.email') }}">
                        ...
                        {{-- Password field --}}
                        <div class="input-group mb-3">
                            <input type="password" name="password" value="{{ $profile->password }}" class="form-control @error('password') is-invalid @enderror"
                        ...
                        <div class="input-group mb-3">
                            <input type="password" name="password_confirmation" value="{{ $profile->password }}"
                        ...
                        <div class="input-group mb-3">
                            <input type="number" name="whatsapp" class="form-control @error('whatsapp') is-invalid @enderror"
                                value="{{ old('whatsapp', $profile->whatsapp) }}" placeholder="923150133533">
