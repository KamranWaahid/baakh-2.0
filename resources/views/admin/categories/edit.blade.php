                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <select name="content_style" id="content_style" class="form-control @error('content_style') is-invalid @enderror">
                                    ...
                                    @foreach ($content_styles as $item)
                                        <option value="{{ $item }}" @selected($item == $category->content_style)>{{ Str::ucfirst($item) }}</option>
                                    @endforeach
                                </select>
                                @error('content_style')
                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="form-group col-4">
                                <label for="gender">Gender</label>
                                    <select name="gender" id="gender" class="form-control select2 @error('gender') is-invalid @enderror">
                                        <option value="masculine" @selected($category->gender === 'masculine')>Masculine / مذڪر</option>
                                        <option value="feminine" @selected($category->gender === 'feminine')>Feminine / مونث</option>
                                    </select>
                            ...
                            <div class="form-group col-2">
                                ...
                                @endphp
                                <x-adminlte-input-switch name="is_featured" data-on-color="success"  :config="$switchConfig" data-on-text="YES" data-off-text="NO" data-off-color="danger"/>
                            </div>
                        </div>
                            ...
                            @foreach ($languages as $i => $lang)
                                <div class="row mt-1 rounded p-2" id="catrow_{{ $i }}" style="background: #9d9ddc2d">
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" 
                                            class="form-control  @error('cat_name' . $i) is-invalid @enderror" 
                                            @if (isset($details[$i]) && $details[$i]->lang == $lang->lang_code)
                                                value="{{ $details[$i]->cat_name }}"
                                            @endif
                                            name="cat_name[]" id="cat_name_{{ $i }}" placeholder="Enter Category Name">
                                    ...
                                    <div class="form-group col-12">
                                        ...
                                        <input type="text" name="cat_name_plural[]" 
                                            class="form-control @error('cat_name_plural' . $i) is-invalid @enderror" id="cat_name_plural_{{ $i }}" 
                                            @if (isset($details[$i]) && $details[$i]->lang == $lang->lang_code)
                                                value="{{ $details[$i]->cat_name }}"
                                            @endif
