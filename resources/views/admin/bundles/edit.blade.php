...
<form action="{{ route('admin.bundle.update', $bundle->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
                    ...
                    <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-6">
                                    ...
                                    @php
                                        $old_title = (!is_null($translations)) ? $translations->title : $bundle->title;
                                    @endphp
                            ...
                            <div class="mb-3">
                              ...
                              <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" rows="3">{{ old('description', $bundle->description) }}</textarea>
                                @error('description')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="dyn-bundle-items" style="border-top:2px dotted solid black;font-family: 'MB Lateefi SK 2.0'; " id="dyn-bundle-items" >
                                @if (old('reference_id'))
                                    @foreach (old('reference_id') as $key => $item)
                                        <div class="couplet-container rounded p-2 mt-3 position-relative" id="item_{{ old('reference_id')[$key] }}" style="background-color:#f4f4f4; border: 1px solid #ccc; padding: 10px;">
                                            <button type="button" data-cid="item_{{ old('reference_id')[$key] }}" class="btn btn-sm btn-danger  btn_remove_couplet position-absolute top-0 m-2" style="right: 0;" data-toggle="tooltip" title="Delete Item"><i class="fa fa-trash"></i></button>
                                            <div class="poetry text-center" style="font-size:1.3rem;">
                                                <p dir="rtl">{{ old('reference_text')[$key] }}</p> 
                                            </div>
                                            <input type="hidden" value="{{ old('reference_id')[$key] }}" name="reference_id[]">
                                            <input type="hidden" value="{{ old('reference_type')[$key] }}" name="reference_type[]">
                                            <input type="hidden" value="{{ old('reference_text')[$key] }}" name="reference_text[]">
                                        </div>
                                    @endforeach
                                @else
                                    @foreach ($items as $key => $item)
                                    <div class="couplet-container rounded p-2 mt-3 position-relative" id="item_{{ $item->reference_id }}" style="background-color:#f4f4f4; border: 1px solid #ccc; padding: 10px;">
                                        <button type="button" data-cid="item_{{ $item->reference_id }}" class="btn btn-sm btn-danger  btn_remove_couplet position-absolute top-0 m-2" style="right: 0;" data-toggle="tooltip" title="Delete Item"><i class="fa fa-trash"></i></button>
                                        <div class="poetry text-center" style="font-size:1.3rem;">
                                            @if ($item->reference_type === 'App\Models\Couplets')
                                                <p>شعر</p>
                                                <p dir="rtl">{!! nl2br($item->reference->couplet_text) !!}</p>
                                                <input type="hidden" value="{{ $item->reference->couplet_text }}" name="reference_text[]">
                                            @else
                                                <p>غزل</p>
                                                <p dir="rtl">{{ $item->reference->poetry_title }}</p>
                                                <input type="hidden" value="{{ $item->reference->poetry_title }}" name="reference_text[]">
                                            @endif
                                        </div>
                                        <input type="hidden" value="{{ $item->reference_id }}" name="reference_id[]">
                                        <input type="hidden" value="{{ $item->reference_type }}" name="reference_type[]">
                                    </div>
                                    @endforeach
                                @endif
                            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group d-flex justify-content-between">
                        <input type="checkbox" name="is_featured" id="statusSwitchis_featured" data-bootstrap-switch data-on-text="Featured" data-off-text="Unfeatured" data-on-color="success" data-off-color="warning" value="1" {{ $bundle->is_featured ? 'checked' : '' }}>
                        <input type="checkbox" name="is_visible" id="statusSwitchis_visible" data-bootstrap-switch data-on-text="Online" data-off-text="Offline" data-on-color="success" data-off-color="danger" value="1" {{ $bundle->is_visible ? 'checked' : '' }}>
                        @error('is_featured')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="form-group">
                       ...
                       <x-adminlte-select2 id="bundle_type" name="bundle_type" label="Bundle Content Type" label-class="text-info" igroup-size="sm" :config="$config">
                            @foreach ($poetry_types as $item)
                                <option value="{{ $item }}" @if (old('bundle_type', $bundle->bundle_layout) == $item)
                                    selected
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="thumbnail-preview">
                        <img src="{{ asset($bundle->bundle_thumbnail) }}" class="img-fluid" alt="">
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="cover-preview">
                        @if ($bundle->bundle_cover)
                            <img src="{{ asset($bundle->bundle_cover) }}" class="img-fluid" alt="">    
                        @endif
                    </div>
                </div>
            </div>
