...
@section('title', 'Edit Poet')

@section('content_header')
    <h1 class="m-0 text-dark">Edit Poet</h1>
    <div class="row">
        @if ($errors->any())
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('admin.poets.update', $poet->id) }}" method="post" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-6">
                                ...
                                <input type="text" class="form-control  @error('poet_slug') is-invalid @enderror"
                                    value="{{ old('poet_slug', $poet->poet_slug) }}" name="poet_slug" id="poet_slug"
                                    placeholder="Enter Poet's Slug or name without space">
                            ...
                            <div class="col-6">
                                ...
                                @php
                                    $selectedTags = old('poet_tags', json_decode($poet->poet_tags) ?: []);
                                    $config = [
                                        "placeholder" => "Select multiple options...",
                                        "allowClear" => true,
                                    ];
                                @endphp
                                <x-adminlte-select2 id="poet_tags" name="poet_tags[]" label="Tags" label-class="text-danger"
                                    ...
                                    @foreach ($tags as $item)
                                        <option value="{{ $item->slug }}" @if (in_array($item->slug, $selectedTags)) selected
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-4">
                                ...
                                <input type="date" class="form-control  @error('date_of_birth') is-invalid @enderror"
                                    value="{{ old('date_of_birth', $poet->date_of_birth) }}" name="date_of_birth"
                                    id="date_of_birth">
                            ...
                            <div class="form-group col-4">
                                ...
                                <input type="date" class="form-control  @error('date_of_death') is-invalid @enderror"
                                    value="{{ old('date_of_death', $poet->date_of_death) }}" name="date_of_death"
                                    id="date_of_death">
                            ...
                            <div class="form-group col-4">
                                <a href="{{ asset($poet->poet_pic) }}" class="float-right" target="_blank">See Current
                                    Picture</a>
                                <x-adminlte-input-file name="image" label="Upload file" placeholder="Choose a file..."
                                    disable-feedback />
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between bg-secondary p-1 rounded">
                            ...
                            <button type="button" class="btn btn-default addMoreRow" onclick="addRow()"><i
                                    class="fa fa-plus"></i></button>
                        </div>

                        <div class="dynamic_details mt-2" id="dynamic_details">
                            @include('admin.poets.dynamic-fields', ['details' => $details])
                        </div>
                    ...
                    <div class="card-footer">
                        ...
                        <button type="button" onclick="window.location.href='{{ route('admin.poets.index') }}'"
                            class="btn btn-warning"><i class="fa fa-arrow-left"></i> Back</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            $(".old_cities_info").each(function () {
                show_cities($(this).data('lang'), $(this).data('row'), $(this).data('birth'), $(this).data('death'))
            });
            // ajax for province
        ...
        function new_row(number) {
            var html = '<div class="row p-2 rounded mt-3" data-cusid="' + number + '" id="dyn_row' + number + '" style="background:#d1e9e4;">' +
                ...
                '<input type="text" class="form-control"  name="poet_name[]" id="poet_name_' + number + '" placeholder="Enter Poet Name">' +
                '</div>' +
                '<div class="form-group col-6">' +
                '<label for="poet_laqab_' + number + '">Laqab</label>' +
                '<input type="text" class="form-control"  name="poet_laqab[]" id="poet_laqab_' + number + '" placeholder="Enter Laqab">' +
                '</div>' +
                '<div class="form-group col-6">' +
                ...
                '<select name="lang[]" id="lang_' + number + '" data-id="' + number + '" class="form-control changeLanguage">' +
        ...
        function show_cities(lang, rowId, birth_selected, death_selected) {
            console.log("show_cities called with lang=" + lang + ", rowId=" + rowId + ", birth_selected=" + birth_selected + ", death_selected=" + death_selected + "");
            var b_url = "{{ url('/admin/') }}";
                ...
                $.get(b_url + '/getCitiesByLang/' + lang, function (data) {
                    $.each(data, function (key, value) {
                        deathPlace.append($('<option>', {
                            ...
                            selected: value.id === death_selected
                        }));
                    ...
                    $.each(data, function (key, value) {
                        birthPlace.append($('<option>', {
                            value: value.id,
                            text: value.city_name,
                            selected: value.id === birth_selected
                        }));
                    });
                });
            }
